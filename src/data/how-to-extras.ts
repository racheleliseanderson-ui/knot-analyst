...
export const HOW_TO_EXTRAS: Record<string, HowTo> = {
  ...
  "cleat-hitch": ht(
    ...
    {
      1: { expectedResult: "Full turn around the base on the far horn.", detail: "The far horn takes the first turn so the lock can still break. Near-horn first turn is the usual jam under surge." },
      2: { expectedResult: "Figure-eights across both horns.", detail: "One clean figure-eight is the working finish; a pile of extra locks welds under surge." },
      3: { expectedResult: "Twisted locking hitch under the last cross, parts parallel.", detail: "The lock must still break by hand with load on the line — a jammed hitch is a knife job." },
      4: { expectedResult: "Dressed hitch that still casts off under load.", detail: "If you cannot cast off while the boat is still loading the line, remake the hitch now." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "round-turn-two-half-hitches": ht(
    ...
    {
      1: { expectedResult: "Full round turn on the bollard or ring.", detail: "The round turn carries surge; half hitches without that turn walk as soon as the load cycles." },
      2: { expectedResult: "First half hitch around the standing part.", detail: "This hitch only locks the turn. It does not carry — do not skip the turn to save rope." },
      3: { expectedResult: "Second half hitch nested in the same direction.", detail: "Both half hitches must face the same way so they nest; opposite directions walk under surge." },
      4: { expectedResult: "Hitches dressed up to the turn.", detail: "Dress the pair hard against the turn before you walk away — floating hitches walk under surge." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "clove-hitch": ht(
    ...
    {
      1: { expectedResult: "First turn around the spar.", detail: "The first turn is only half the clove; leave it stacked so the second turn can cross it." },
      2: { expectedResult: "Second turn crossed over the first.", detail: "The second turn must cross over the first. Same-side turns are not a clove and will not hold." },
      3: { expectedResult: "Working end tucked under the second turn, both turns stacked.", detail: "A spread clove is already walking — dress both turns tight together before you leave it." },
      4: { expectedResult: "Watched hitch, or backed up if the load will cycle.", detail: "Tide or chop means a different hitch. Leaving an unattended clove on a rail is the wrong job." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "anchor-bend": ht(
    ...
    {
      1: { expectedResult: "Round turn through the ring.", detail: "A single pass through the ring is not this hitch — the round turn is what takes swell." },
      2: { expectedResult: "Working end tucked through both turns.", detail: "The working end must pass through both ring turns; missing one leaves a slip in a swell." },
      3: { expectedResult: "Half hitch on the standing part.", detail: "The half hitch only locks the rode; it does not replace the round turn on the ring." },
      4: { expectedResult: "Hard dress. Tail seized if this stays on the ring.", detail: "A rode that lives underwater gets seized or taped — an unseized soak can work the tail loose." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  bowline: ht(
    ...
    {
      1: { expectedResult: "Small loop in the standing part, working end up through it.", detail: "Wrong loop direction is a cowboy bowline. The working end must go up through the small loop, around the standing part, and back down." },
      2: { expectedResult: "Working end around the standing part.", detail: "The working end must go around the standing part so the collar can sit on it — around the eye never forms a collar." },
      3: { expectedResult: "Working end back down through the small loop.", detail: "Leaving the tail up is unfinished — it must return down through the nipping loop to lock." },
      4: { expectedResult: "Collar tight, tail long enough, backup if the job is serious.", detail: "An open collar can capsize under cyclic load. Dress it tight and backup the tail on a serious job." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "figure-8-loop": ht(
    ...
    {
      1: { expectedResult: "Bight long enough for the finished eye.", detail: "Eye size is decided in the bight before the eight. Too short and the eight eats the loop." },
      2: { expectedResult: "Clean figure-eight in the bight.", detail: "Both strands must nest through the same eight. A twisted or crossed eight will not dress and is weaker." },
      3: { expectedResult: "Both parts nested, eye the size you wanted.", detail: "You cannot resize after a snatch — dress the eye now, before the eight is set hard." },
      4: { expectedResult: "Both strands parallel and inspectable.", detail: "A crossed eight is a different, weaker knot. Parallel inspectable strands, or retie." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "sheet-bend": ht(
    ...
    {
      1: { expectedResult: "Bight in the thicker or standing rope.", detail: "The bight belongs in the thicker rope. A bight in the thin rope is the usual spill under load." },
      2: { expectedResult: "Second rope up through the bight.", detail: "Wrong first direction is still recoverable — continue around both parts of the bight and tuck." },
      3: { expectedResult: "Around both parts, tucked under its own standing part.", detail: "The working end must tuck under its own standing part. Missing that tuck is a hitch, not a bend." },
      4: { expectedResult: "Both tails same side, dressed.", detail: "Opposite tails are a left-handed sheet bend and will slip. Same-side tails are the lock." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "double-sheet-bend": ht(
    ...
    {
      1: { expectedResult: "Bight in the thicker rope.", detail: "The thick rope always makes the bight. A thin-rope bight will spill as soon as the mismatch loads." },
      2: { expectedResult: "Thinner rope up through the bight.", detail: "Leave working room for two turns of the thinner rope — one turn is the wrong job on a mismatch." },
      3: { expectedResult: "Two turns around the bight, then the tuck.", detail: "One turn is a single sheet bend on a mismatch job. The extra turn is why this hitch exists." },
      4: { expectedResult: "Hard dress, tails same side.", detail: "Opposite tails slip the same way as a left-handed sheet bend. Same-side tails after the second turn." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "rolling-hitch": ht(
    ...
    {
      1: { expectedResult: "Two turns on the load side of the standing part.", detail: "Both first turns must sit on the load side of the standing part. Turns on the wrong side will not grip." },
      2: { expectedResult: "Third turn crossing.", detail: "The third turn is what makes it a rolling hitch. A clove here walks under along-line pull." },
      3: { expectedResult: "Working end tucked under the last turn.", detail: "The tail tucked under the last turn sets the direction — without it the hitch is not finished." },
      4: { expectedResult: "Dressed toward the load; slide-test holds.", detail: "Slide-test toward the load before you trust it. If it walks, add a turn or change hitch — do not leave it." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "figure-8-stopper": ht(
    ...
    {
      1: { expectedResult: "First loop of the figure-eight in the tail.", detail: "An overhand here is a different, jammier stopper. Form a true figure-eight so it can still be inspected and untied." },
      2: { expectedResult: "Eight completed, tail through.", detail: "An unfinished or loose eight pulls through the block. Complete the eight and dress it compact." },
      3: { expectedResult: "Compact eight larger than the opening.", detail: "Size after dress is the job — the eight must be larger than the block or fairlead." },
      4: { expectedResult: "Short tail. Not loaded as a join.", detail: "A stopper is not a bend. If you need a join, pick a sheet bend or Zeppelin." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "water-bowline": ht(
    ...
    {
      1: { expectedResult: "Two overlapping loops sitting as a clove hitch in the standing part.", detail: "Those two overlapping loops must sit as a clove hitch — that clove is the water collar, not extra bulk." },
      2: { expectedResult: "Working end up through both clove loops.", detail: "The working end must pass up through both clove loops. Missing a loop is an unfinished water bowline." },
      3: { expectedResult: "Around the standing part and back down the clove.", detail: "Leaving the tail up is a slip. It must go around the standing part and back down through the clove." },
      4: { expectedResult: "Clove dressed tight against the standing part.", detail: "A loose clove is just a fat bowline. Dress the clove tight against the standing part before you trust the eye." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "bowline-on-a-bight": ht(
    ...
    {
      1: { expectedResult: "Long bight where the loops must sit.", detail: "Start with a long bight. A short bight cannot open and pass around the whole knot." },
      2: { expectedResult: "Bight passed through a loop in the doubled rope.", detail: "The bight through the doubled loop is only half the knot — it still has to encircle both standing parts." },
      3: { expectedResult: "Opened bight passed around the whole knot.", detail: "The opened bight must pass around the whole knot. If it sits on one standing part only, restart." },
      4: { expectedResult: "Two dressed loops, bight around both standings.", detail: "Unequal undressed loops will capsize. Dress both eyes after the wrap-around before you load them." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "alpine-butterfly": ht(
    ...
    {
      1: { expectedResult: "Two wraps on the hand — fingertip turn, then thumb turn.", detail: "Two wraps on the hand — fingertip then thumb. One wrap is not an Alpine Butterfly." },
      2: { expectedResult: "Fingertip turn wrapped around the other two.", detail: "Lift the fingertip turn around the other two. Lifting the wrong turn inverts the body into a slip loop." },
      3: { expectedResult: "Structure slid off with wrap order intact.", detail: "Slide the structure off with wrap order intact. A scramble off the hand is a retie, not a dress." },
      4: { expectedResult: "Square body, loop standing, both standings leaving opposite sides.", detail: "Load the loop and both standing parts so the body sits square. A twisted body is a retie." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "cow-hitch": ht(
    ...
    {
      1: { expectedResult: "Bight around the post or through the ring.", detail: "Use a bight around the post or through the ring. A single pass is not a cow hitch." },
      2: { expectedResult: "Both legs through the bight.", detail: "Both legs must go through the bight. One leg through is a girth that will walk under the first snatch." },
      3: { expectedResult: "Hitch square, both legs leaving together.", detail: "Dress the hitch square with both legs leaving together. A twisted hitch is already walking." },
      4: { expectedResult: "Both legs loaded.", detail: "Load both legs. If only one leg will take the strain, this hitch will walk — switch hitch." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "buntline-hitch": ht(
    ...
    {
      1: { expectedResult: "Working end through the ring.", detail: "Leave enough tail for a clove hitch on the standing part. A short tail cannot put the second turn inside." },
      2: { expectedResult: "Clove on the standing part, second turn toward the ring.", detail: "The second clove turn must sit toward the ring. An outside turn is two half hitches, not a buntline." },
      3: { expectedResult: "Clove dressed up to the ring.", detail: "Dress the clove up to the ring. A floating clove will walk, then jam in the wrong place." },
      4: { expectedResult: "Standing attachment you do not expect to break by hand.", detail: "This is a standing attachment you do not expect to break by hand. Need a release? Use round turn and two half hitches." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "icicle-hitch": ht(
    ...
    {
      1: { expectedResult: "Four or five turns away from the pull.", detail: "Four or five turns away from the pull. Too few turns on a slick or tapered surface walk." },
      2: { expectedResult: "Working end back alongside the standing part, bight behind.", detail: "Bring the working end back and leave a bight behind the spar — no bight means you cannot finish the hitch." },
      3: { expectedResult: "Bight over both ends and over the spar end.", detail: "The bight must pass over both ends and over the spar end. Missing the spar end is unfinished." },
      4: { expectedResult: "Set perpendicular, then along-spar load holds.", detail: "Set perpendicular first, then load along the spar. If it walks on the slide-test, add a turn — do not trust it." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "truckers-hitch": ht(
    ...
    {
      1: { expectedResult: "Line around the far anchor and back.", detail: "The line must go around a far anchor and back. No far point, no 2:1 purchase." },
      2: { expectedResult: "Directional loop in the standing part facing the purchase.", detail: "The directional loop must face the purchase and stay standing. A collapsing loop dumps the tension." },
      3: { expectedResult: "Tail through the loop, line under working tension.", detail: "Pull the tail through the loop first — that is the 2:1. Locking before tension wastes the purchase." },
      4: { expectedResult: "Two nested half hitches locking the tension.", detail: "Lock with two nested half hitches before you walk away. No lock, or opposite-direction hitches, and the line goes slack." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "zeppelin-bend": ht(
    ...
    {
      1: { expectedResult: "First rope looped like a 6, working end on top.", detail: "The first rope loops like a 6 with the working end on top. That 6 is half the lock — a random bight is not it." },
      2: { expectedResult: "Second rope looped like a 9, working end opposite.", detail: "The second rope loops like a 9 with the working end opposite. Same-side ends will not lock." },
      3: { expectedResult: "Each working end through the centre of both loops.", detail: "Each working end must pass through the centre of both loops. Missing a loop is an unfinished Zeppelin." },
      4: { expectedResult: "Two interlocked loops, tails opposite, standing parts loaded.", detail: "The finished lock is two interlocked loops with tails opposite. If it looks like a sheet bend, restart." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "carrick-bend": ht(
    ...
    {
      1: { expectedResult: "Loop in the first rope, tail under its standing part.", detail: "First loop with the tail under its standing part. Tail over is the wrong lattice start and will not weave." },
      2: { expectedResult: "Second rope woven under-over through that loop.", detail: "The second rope weaves under-over through that loop. Broken over-under is a tangle, not a Carrick — count the overs." },
      3: { expectedResult: "Second tail across the loop and under itself.", detail: "The second tail must go across the loop and under itself. Missing that under dumps the lattice." },
      4: { expectedResult: "Standing parts pulled; tails seized if this stays.", detail: "Pull both standing parts, then seize both tails on a standing hawser. An unseized Carrick can walk." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "ashley-stopper": ht(
    ...
    {
      1: { expectedResult: "Loose overhand, long working tail.", detail: "Leave a long working tail. A short tail cannot form the third lobe and you are left with an overhand." },
      2: { expectedResult: "Tail back through the overhand on the documented path.", detail: "The tail must return through the overhand on the documented path. Wrong return stays an overhand." },
      3: { expectedResult: "Collar showing three distinct lobes.", detail: "Three distinct lobes on the underside. Two lobes means keep following — it is still an unfinished overhand." },
      4: { expectedResult: "Compact three-lobe stopper larger than the opening.", detail: "Dress compact and larger than the opening. If it pulls through, it is unfinished or the wrong size." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "yosemite-bowline": ht(
    ...
    {
      1: { expectedResult: "Dressed bowline, collar around the standing part.", detail: "Bowline first — collar around the standing part. An unfinished bowline cannot take a Yosemite finish." },
      2: { expectedResult: "Tail back along the standing part and down through the nipping loop.", detail: "The tail must go back along the standing part and down through the nipping loop. Tail left out is just a bowline." },
      3: { expectedResult: "Collar seated first, then the tuck.", detail: "Dress the collar first, then the tuck. Reverse that order and the Yosemite finish can invert the eye." },
      4: { expectedResult: "Tail parallel inside the nipping loop.", detail: "Finished tail lies parallel inside the nipping loop. Short tail or a loose collar is a retie." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "pile-hitch": ht(
    ...
    {
      1: { expectedResult: "Long bight dropped over the pile.", detail: "Fold a long bight. A short bight cannot reclear the top of the pile and the hitch will walk off." },
      2: { expectedResult: "Bight passed around both standing parts.", detail: "Pass the bight around both standing parts. Around one standing part only will walk under surge." },
      3: { expectedResult: "Bight dropped back over the top.", detail: "Drop the bight back over the top of the pile. Never recleared is unfinished and will walk off." },
      4: { expectedResult: "Both legs loaded. Temporary stay.", detail: "Both legs loaded, temporary stay. Leaving it through a tide is the wrong hitch — switch to round turn and two half hitches." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "halyard-hitch": ht(
    ...
    {
      1: { expectedResult: "Working end through the shackle.", detail: "Leave enough tail for two turns on the standing part and the return tuck. A short tail cannot finish a halyard hitch." },
      2: { expectedResult: "Two turns around the standing part.", detail: "Two turns around the standing part. One turn will walk on the shackle as soon as the hoist loads." },
      3: { expectedResult: "Tail tucked back through the turns toward the shackle.", detail: "Tuck the tail back through the turns toward the shackle. Tuck the other way is unfinished." },
      4: { expectedResult: "Compact against the pin. Expect a jam.", detail: "Dress hard against the pin and expect a jam. If you must undo it after a hoist, pick another hitch." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "midshipmans-hitch": ht(
    ...
    {
      1: { expectedResult: "First turn on the load side, tucked under itself.", detail: "The first turn on the load side must tuck under itself. That inside tuck is the hitch — missing it is a taut-line." },
      2: { expectedResult: "Second turn beside it, still on the load side.", detail: "The second turn sits beside the first, still on the load side. Turns away from the load will not grip." },
      3: { expectedResult: "Half hitch on the outside, away from the load.", detail: "Finish with a half hitch on the outside, away from the load. No outside hitch and it walks back." },
      4: { expectedResult: "Slides to tension; holds a light pull.", detail: "Slide to tension and test a light pull. If it walks, the inside tuck is missing — that is a taut-line, not this." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "hunters-bend": ht(
    ...
    {
      1: { expectedResult: "Tails overlapped opposite ways, lying parallel.", detail: "Overlap the tails opposite ways so they lie parallel. A 6-and-9 start is a Zeppelin, not Hunter's." },
      2: { expectedResult: "Each tail around both ropes and through its own loop.", detail: "Each tail goes around both ropes and back through its own loop. Missing a loop is an unfinished Hunter's." },
      3: { expectedResult: "Working ends exit opposite.", detail: "Working ends must exit opposite. Same-side tails will not lock a Hunter's bend." },
      4: { expectedResult: "Compact double-overhand body, not a 6-and-9.", detail: "Dress into the compact double-overhand body. If it looks like a 6-and-9, you tied a Zeppelin — keep it on purpose or retie." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "heaving-line-knot": ht(
    ...
    {
      1: { expectedResult: "Long bight at the end of the heaving line.", detail: "Form a long bight at the end. A short bight cannot take enough wraps to carry." },
      2: { expectedResult: "Working end wrapped toward the bight end.", detail: "Wrap toward the bight end. More wraps, more mass — too few wraps will not carry to the dock." },
      3: { expectedResult: "Tail through the remaining bight.", detail: "Pass the tail through the remaining bight. Tail left out dumps the wraps in the air." },
      4: { expectedResult: "Wraps packed. Used only to throw.", detail: "This is weight to throw, not a stopper for a block. Asked to take load, pick figure-8, Ashley, or Estar." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "estar-stopper": ht(
    ...
    {
      1: { expectedResult: "First loop of the documented Estar path.", detail: "Follow the documented Estar path. A figure-8 start is the wrong stopper in HMPE and will walk." },
      2: { expectedResult: "Extra turns a figure-8 does not have.", detail: "Take the extra turns a figure-8 does not have. Those tucks are why this stopper holds in Dyneema." },
      3: { expectedResult: "Tail through so the body cannot walk.", detail: "Follow the tail through so the body cannot walk. An open or unfinished Estar slides in HMPE." },
      4: { expectedResult: "Compact stopper larger than the opening.", detail: "Dress compact and larger than the opening. Pulls through if unfinished, loose, or the wrong size." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "reef-knot": ht(
    ...
    {
      1: { expectedResult: "First half-knot around the sail or bundle.", detail: "First half-knot around the sail or bundle. This is a bind, not a join of two ropes — a reef used as a bend will slip." },
      2: { expectedResult: "Opposite-hand second half-knot.", detail: "Second half-knot is the opposite hand. Same hand twice is a granny and will slip as soon as the reef takes strain." },
      3: { expectedResult: "Flat, two nested half-knots.", detail: "The knot must sit flat, two nested half-knots. A capsized reef is already failing as a bind." },
      4: { expectedResult: "Binding only. Standing parts are not a join.", detail: "Binding only. If the two standing parts will take load as a join, this will slip — pick a sheet bend or Zeppelin." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  constrictor: ht(
    ...
    {
      1: { expectedResult: "Around the object, crossed over the standing part.", detail: "Pass around the object and cross over the standing part. A single pass is not a constrictor." },
      2: { expectedResult: "Second turn around the object.", detail: "Around the object a second time. One turn cannot form the crossed riding lock." },
      3: { expectedResult: "Tail under the standing part and the first turn.", detail: "Tuck the tail under the standing part and the first turn. Missing that tuck is a clove, not a constrictor." },
      4: { expectedResult: "Dressed hard. Plan to cut it.", detail: "Dress hard and plan to cut it. If you must release, this is the wrong bind — use a reef knot." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
  "timber-hitch": ht(
    ...
    {
      1: { expectedResult: "Around the spar, working end back along the standing part.", detail: "Around the spar, working end back along the standing part. No standing part to dog around is unfinished." },
      2: { expectedResult: "Three or more tucks dogged around that standing part.", detail: "Dog the working end with three or more tucks. A single tuck walks as soon as the spar turns." },
      3: { expectedResult: "Tucks biting the standing part.", detail: "Dress so the tucks bite the standing part. Loose dogs dump under the first hoist." },
      4: { expectedResult: "Loaded. Collapses when slack — that is correct.", detail: "Load it. Unloaded it collapses — that is the hitch. Need it to hold slack? Wrong hitch — use rolling or icicle." },
    },
    ROPE_NOTES,
    ROPE_DRESS,
  ),
...
export const MICRO_EXTRAS: Record<string, Micro> = {
  ...
  "bowline-on-a-bight": mx({
    1: ["Long mid-line bight.", "Too short to finish the wrap-around.", "Start with a longer bight so it can encircle both standings."],
    2: ["Bight through the loop in the doubled rope.", "This is only half the knot.", "Continue — the bight still has to pass around both standing parts."],
    3: ["Opened bight around the whole knot.", "Sitting on one standing part is unfinished.", "Pass the opened bight around both standing parts."],
    4: ["Two loops, bight around both standings.", "A one-sided bight will capsize.", "Retie and finish the wrap-around."],
  }),
  "alpine-butterfly": mx({
  ...
  "yosemite-bowline": mx({
    1: ["Dressed bowline, collar around the standing part.", "Unfinished bowline cannot take the finish.", "Finish the bowline first."],
    2: ["Tail back along the standing part, down the nipping loop.", "Tail left out is just a bowline.", "Complete the Yosemite tuck."],
    3: ["Collar seated first, then the tuck.", "Tuck-first dress can invert the eye.", "Dress collar, then tuck."],
    4: ["Tail parallel inside the nipping loop.", "Short tail or loose collar is a retie.", "Retie and leave more tail."],
  }),
  "pile-hitch": mx({
    ...
    3: ["Bight dropped back over the top.", "Never recleared is unfinished.", "Drop it over the top."],
    4: ["Both legs loaded. Temporary.", "Overnight tide is the wrong hitch.", "Switch to round turn and two half hitches."],
  }),
  "halyard-hitch": mx({
    1: ["Working end through the shackle.", "Short tail cannot finish the turns.", "Leave more tail."],
    2: ["Two turns on the standing part.", "One turn walks.", "Add the second turn."],
    3: ["Tail tucked back toward the shackle.", "Tuck the other way is unfinished.", "Tuck toward the pin."],
    4: ["Compact against the pin. Expect a jam.", "Need a release? Wrong hitch.", "Use round turn and two half hitches."],
  }),
  "midshipmans-hitch": mx({
    1: ["First turn on the load side, tucked under itself.", "Missing the inside tuck is a taut-line.", "Tuck the first turn under itself."],
    2: ["Second turn beside it, load side.", "Turns away from the load will not grip.", "Restart on the load side."],
    3: ["Outside half hitch away from the load.", "No outside hitch and it walks back.", "Add the half hitch."],
    4: ["Slides to tension; holds a light pull.", "Walks? Inside tuck is missing.", "Retie the inside tuck."],
  }),
  "hunters-bend": mx({
    1: ["Tails overlapped opposite, lying parallel.", "A 6-and-9 start is the other bend.", "Overlap, do not loop a 6."],
    2: ["Each tail around both and through its own loop.", "Missing a loop is unfinished.", "Through its own loop."],
    3: ["Working ends exit opposite.", "Same-side tails will not lock.", "Flip one tail."],
    4: ["Compact double-overhand body.", "Looks like a 6-and-9? You tied a Zeppelin.", "Retie Hunter's or keep the Zeppelin on purpose."],
  }),
  "heaving-line-knot": mx({
    ...
    4: ["Wraps packed. Throw only.", "Asked to take load — wrong stopper.", "Pick figure-8 / Ashley / Estar."],
  }),
  "estar-stopper": mx({
    1: ["First loop of the Estar path.", "A figure-8 start is the wrong stopper in HMPE.", "Follow the Estar path."],
    2: ["Extra turns a figure-8 does not have.", "Those tucks are the hold in Dyneema.", "Add the extra tucks."],
    3: ["Tail through so the body cannot walk.", "An open body slides.", "Finish the path."],
    4: ["Compact stopper larger than the opening.", "Pulls through if unfinished or too small.", "Retie larger."],
  }),
  "reef-knot": mx({
    1: ["First half-knot around the sail or bundle.", "This is a bind, not a join.", "Bind the reef, do not join two ropes."],
    2: ["Opposite-hand second half-knot.", "Same hand twice is a granny.", "Reverse the second half."],
    3: ["Flat nested half-knots.", "A capsized reef is already failing.", "Dress it flat."],
    4: ["Binding only.", "Standing parts loaded as a join will slip.", "Pick a bend if you needed a join."],
  }),
  constrictor: mx({
    1: ["Around the object, crossed over the standing part.", "A single pass is not this bind.", "Cross over."],
    2: ["Second turn around the object.", "One turn cannot form the riding lock.", "Go around again."],
    3: ["Tail under standing part and first turn.", "Missing that tuck is a clove.", "Tuck under both."],
    4: ["Dressed hard. Plan to cut it.", "Need a release? Wrong bind.", "Use a reef knot if you must untie."],
  }),
  "timber-hitch": mx({
    1: ["Around the spar, working end back along the standing part.", "No standing part to dog is unfinished.", "Bring it back along."],
    ...
    4: ["Loaded. Collapses when slack — correct.", "Need slack hold? Wrong hitch.", "Use rolling or icicle."],
  }),
};
