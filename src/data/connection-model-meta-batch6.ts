...
export const CONNECTION_MODEL_META_BATCH6: Record<string, ConnectionModelMeta> = {
  bristol: {
    ...
    strengthRetentionBand: band(
      70,
      85,
      "Tom Rowland No-Name/Bristol braid-to-fluoro tests averaged ~77%. Band is that test spread when the double is sound.",
    ),
  ...
  "double-double-uni": {
    ...
    strengthRetentionBand: band(
      75,
      90,
      "Uni-join family with a doubled braid barrel. More grip than Double Uni; not an FG-slim figure.",
    ),
    failsWhen: [
      "Braid not doubled through the barrel",
      "Too few braid wraps",
      "One Uni closes first",
    ],
  ...
  "loop-to-loop": {
    ...
    strengthRetentionBand: band(
      70,
      95,
      "Join is only as honest as the weaker loop. No independent retention figure for the handshake itself.",
    ),
  ...
  willis: {
    ...
    strengthRetentionBand: band(
      60,
      85,
      "Finger-trap on an intact lead-core sheath. No published single figure; band is insertion-length dependent.",
    ),
  ...
  "kryston-loop": {
      ...
      invalidPairs: [
        {
          main: "fluoro",
          secondary: "fluoro",
          reason: "NetKnots material notes: poor in fluorocarbon",
        },
      ],
    },
    ...
    strengthRetentionBand: band(
      80,
      95,
      "NetKnots: tests well in mono, poorly in fluoro. Band is the mono result only.",
    ),
